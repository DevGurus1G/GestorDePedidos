    ...
    <div class="row mt-4">
      ...
      <form>
        ...
        <div class="mb-3">
          ...
          <input type="text" class="form-control" id="codigo" aria-describedby="ayuda">
          <div id="ayudaCodigo" class="form-text">Si desea cambiar su codigo de acceso pongase en contacto con
            el administrador
          </div>
        ...
        <div class="mb-3">
          ...
          <div id="ayudaDni" class="form-text">Si desea cambiar su dni pongase en contacto con el administrador
          </div>
        </div>
        <button type="Actualizar" class="btn btn-primary">Submit</button>
      </form>
    </div>
...
export default {
  data() {
    return {
      ...
      usuario: {}, // Un array para almacenar los pedidos
    };
  },
  mounted() {
  ...
  methods: {
    async getUsuario() {
      try {
        const response = await fetch('URL_API_PARA_OBTENER_usuario');
        const data = await response.json();

        if (data.success) {
          //Actualiza el objeto usuario con los datos del server
